...
//   public isReachDailyMax = () => this._receipts.length === DailyReceiptModel.DAILY_MAX_RECEIPT;

//   public getCount = () => this._receipts.length;

//   public getDailyTotalCost = (): number => {
//     if (this._receipts.filter((receipt) => receipt.cost === null).length > 0) return null;
//     return this._receipts.reduce((accumulator: number, receipt: ReceiptModel): number => accumulator + receipt.cost, 0);
//   };
...
//   public isExistSameStore = () =>
//     this._receipts.filter((r, i, s) => s.findIndex((s) => s.storeName === r.storeName) === i).length !==
//     this.getCount();

//   public isDuplicate = () =>
//     this._receipts.filter((r, i, s) => s.findIndex((s) => s.cost === r.cost && s.storeName === r.storeName) === i)
