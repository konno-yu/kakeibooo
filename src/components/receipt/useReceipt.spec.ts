...
describe('useReceipt', () => {
  ...
  test('onReceiptAddが呼ばれたらレシートが追加される（0 → 1）', () => {
    const { result } = renderHook(({ initValue }) => useReceipt(initValue), {
      initialProps: {
        initValue: [],
      },
    });
  ...
  test('onReceiptAddが呼ばれたらレシートが追加される（1 → 2）', () => {
    const { result } = renderHook(({ initValue }) => useReceipt(initValue), {
      initialProps: {
        initValue: [{ index: 1, storeName: '', cost: 100 }],
      },
    });
    act(() => {
      result.current.onReceiptAdd();
  ...
  test('onChangeStoreNameで店舗名が変更できる', () => {
    const { result } = renderHook(({ initValue }) => useReceipt(initValue), {
      initialProps: {
        initValue: [{ index: 1, storeName: '1', cost: 100 }],
      },
    });
    act(() => {
  ...
  test('レシートが1枚でもある場合はノーマネーデイとして登録できない', () => {
    const { result } = renderHook(({ initValue }) => useReceipt(initValue), {
      initialProps: {
        initValue: [{ index: 1, storeName: 'a', cost: 100 }],
      },
    });
